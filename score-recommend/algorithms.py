...
def gaussian_regression_f(data_x, data_y, domain_x, theta1, theta2, theta3, theta4, theta5):
    ...
    K = kernel(data_x.copy(), data_x.copy(), theta1, theta2, theta3, theta4, theta5) #+ sig_y * np.eye(len(data_x))
    k_ = kernel(data_x.copy(), domain_x.copy(), theta1, theta2, theta3, theta4, theta5)
...
def recommend_from_score_dict(score_dict: dict) -> list:
    ...
    for id_ in keys:
        ...
        data_x.append(df.loc[df['id'] == id_, 'elem'].values.flatten().tolist())
    ...
    theta4 = 0
    theta5 = 0
    z = gaussian_regression_f(data_x, data_y, domain_x, theta1, theta2, theta3, theta4, theta5)
...
'''sample'''
score_dict = {'1043077': 0.1, '1064775': 0.3, '1052094': 0.1,'1038230': 0.1, '1061347': 0.1,}
