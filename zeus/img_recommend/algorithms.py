...
def lambda_handler(event, context):
    # 全queryを受け取る
    try:
        q=dict(event['queryStringParameters'])
        img_path=q['img_path']
        # ここから AI アルゴリズム実行
        is_face, ids=recommend_from_img_path(img_path)

        # ここまで
        return {
            'statusCode': 200,
            'body': json.dumps({"is_face":is_face,"ids":ids})
        }
    except:
        q = {}
        # ここまで
        return {
            'statusCode': 500,
            'body': json.dumps({"error":"クエリのimg_pathを指定してください"})
        }
